...
- 名刺撮影 → Claude Vision OCR → お礼メール自動生成・送信
- 文脈情報（場所・イベント名・温度感・メモ・写真）の記録
- **複数名刺対応**: 1人のContactに複数社の名刺を追加。チップバーで名刺切替、選択中カードのメアド宛にメール送信
- **SNSマッチング**: 名刺から抽出したSNS × 自分のプロフィールSNSを自動マッチング。ブランドカラーボタンで直リンク・「✓ 繋がった」ボタンで記録
- **重複検出**: 同じメールアドレスの名刺は自動検出し、新たな出会いを追記
- **出会い履歴**: 同一人物と複数回会うたびに履歴として蓄積（写真付き）
- **公開プロフィールページ** (`/p/[userId]`): Linktreeライクなデザイン。SNSフル幅ボタン・所属カード・顔写真、QRコードでアクセス可能
- **QRコード付きHTML署名**: 送信メールに自動でQRコード＋プロフィールURLを挿入
- **プロフィール設定**: SNSリンク21種（personal/business/cardappカテゴリ）・所属情報（最大5件）・顔写真・名刺スキャンからの自動入力
- チーム管理・メンバー招待
