...
export function useRequireAuth() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      // session がある場合のみ init する。
      // null の場合は onAuthStateChange の INITIAL_SESSION イベントに委ねる。
      // こうしないと Vercel 本番で getSession が一瞬 null を返した際に
      // router.replace('/login') が先走りして "Abort fetching component" エラーになる。
      if (mounted && session) init(session)
    })
